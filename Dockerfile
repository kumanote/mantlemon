...
FROM alpine:3.16
COPY --from=builder /mantlemon/target/release/mantlemon /usr/local/bin/mantlemon
RUN chmod +x /usr/local/bin/mantlemon
...
RUN addgroup -g ${GROUP_ID} mantlemon; \
    adduser -D -u ${USER_ID} -G mantlemon -h /mantlemon -s "/bin/bash" mantlemon;

# install su-exec
RUN apk add --no-cache su-exec; \
    su-exec mantlemon true;
