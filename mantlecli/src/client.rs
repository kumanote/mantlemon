...
use anyhow::Context;
use logger::prelude::*;
use mantle_grpc_client as proto;
use mantle_grpc_client::tonic::{self, transport::Channel, Code};

#[derive(Debug, Clone)]
pub struct MantleNodeClient {
    endpoint: String,
    connection: Option<Channel>,
}

impl MantleNodeClient {
    pub async fn establish_connection(endpoint: &str) -> Result<Channel> {
        tonic::transport::Endpoint::new(endpoint.to_owned())?
            ...
            .with_context(|| {
                format!(
                    "failed to connect mantle node daemon grpc endpoint: {}",
                    endpoint
                )
            })
    }
