...
use logger::prelude::*;
use mantle_grpc_client::{self as proto, prost};
...
impl NewProposalChecker {
    pub fn new(mantle_node_endpoint: String, receiver: Receiver<NewProposalMessage>) -> Self {
        Self {
                                                ...
                                                body.messages.iter().enumerate()
                                            {
                                                    ...
                                                    == "/cosmos.gov.v1beta1.MsgSubmitProposal"
                                                {
                                                    ...
                                                    let message_logs = if tx_message_logs.is_some()
                                                    {
                                                        ...
                                                        let tx_response = mantlecli::get_client(
                                                            &self.mantle_node_endpoint,
                                                        )
                                                        .lock()
                                                        .await
