...
pub struct CheckerConfig {
    pub asset_mantle_grpc_scheme: String,
    pub asset_mantle_grpc_host: String,
    pub asset_mantle_grpc_port: u16,
...
impl CheckerConfig {
    pub fn mantle_node_grpc_endpoint(&self) -> String {
        format!(
            "{}://{}:{}",
            self.asset_mantle_grpc_scheme, self.asset_mantle_grpc_host, self.asset_mantle_grpc_port
        )
    }
}
...
impl SelfValidation for CheckerConfig {
    fn validate(&self) -> Result<()> {
        if self.asset_mantle_grpc_scheme != "http" && self.asset_mantle_grpc_scheme != "https" {
            return Err(anyhow!(
                "asset mantle node daemon grpc_scheme must be either 'http' or 'https'..."
            ));
        }
...
impl FromEnv for CheckerConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            asset_mantle_grpc_scheme: "http".to_owned(),
            asset_mantle_grpc_host: "127.0.0.1".to_owned(),
            asset_mantle_grpc_port: 9090,
...
impl TryFrom<CheckerToml> for CheckerConfig {
    ...
    fn try_from(toml: CheckerToml) -> Result<Self> {
        ...
        if let Some(asset_mantle_grpc_scheme) = toml.asset_mantle_grpc_scheme {
            result.asset_mantle_grpc_scheme = asset_mantle_grpc_scheme;
        }
        if let Some(asset_mantle_grpc_host) = toml.asset_mantle_grpc_host {
            result.asset_mantle_grpc_host = asset_mantle_grpc_host;
        }
        if let Some(asset_mantle_grpc_port) = toml.asset_mantle_grpc_port {
            result.asset_mantle_grpc_port = asset_mantle_grpc_port;
        }
