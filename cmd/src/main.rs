...
use logger::prelude::*;
use mantlecli::MantleNodeClient;
...
#[structopt(about = "mantlemon running options")]
struct Opts {
...
fn main() {
    ...
    debug!(
        "Loaded asset mantle node daemon monitoring tool config, config: {:?}",
        config
    );

    // set up mantle node client pool
    let mut client_pool = HashMap::new();
    for checker in &config.checkers {
        let endpoint = checker.mantle_node_grpc_endpoint();
        info!("this tool will connect to {}", endpoint.as_str());
        let client = MantleNodeClient::new(endpoint.clone());
        client_pool.insert(endpoint, Arc::new(Mutex::new(client)));
    }
    mantlecli::set_client_pool(client_pool);

    if let Err(err) = mantlemon::start() {
        error!("{}", err);
    }
